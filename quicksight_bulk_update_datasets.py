...
def rename_schema(
    ...
    def all_dataset_ids(client):
        with Progress(SpinnerColumn(finished_text='[green]✔'), TextColumn("{task.description}")) as progress:
            ...
            import itertools
            dataset_ids = tuple(itertools.islice((
        ...
        for physical_table_id, physical_table in dataset.get("PhysicalTableMap", {}).items():
            ...
            if "RelationalTable" in physical_table:
                ...
                if table["Schema"] == source_schema:
                    hide_name = rename(table['Name'])
                    grid = Table.grid(padding=(0, 1))
                    grid.add_column(justify="left")
                    grid.add_column(justify="left")
                    grid.add_row(f"[green]✔[/green] RelationalTable", ReprHighlighter()(f"{(table['Schema'], hide_name)} ➜ {(target_schema, hide_name)}"))
            ...
            if "CustomSql" in physical_table:
                ...
                if any(schema == source_schema for schema, table in tables):
                    ...
                    original_tables_mapped = [(schema, hide_map[table]) for schema, table in tables]
                    new_tables_mapped = [(schema, hide_map[table]) for schema, table in new_tables]
                    ...
                    innerGrid.add_column(justify="left")
                    for old, new in zip(original_tables_mapped, new_tables_mapped):
